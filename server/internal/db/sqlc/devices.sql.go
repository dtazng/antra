...
func (q *Queries) UpsertDeviceToken(ctx context.Context, arg UpsertDeviceTokenParams) (DeviceToken, error) {
	...
	var i DeviceToken
	err := row.Scan(
		&i.ID, &i.UserID, &i.Token, &i.Platform, &i.IsActive, &i.CreatedAt, &i.UpdatedAt,
	)
	return i, err
}
...
func (q *Queries) DeactivateDeviceToken(ctx context.Context, id, userID uuid.UUID) error {
	_, err := q.db.Exec(ctx, deactivateDeviceToken, id, userID)
	return err
}
